...
public class SkillChargeCondition extends ChargeCondition {
	...
	public boolean validate(Skill env) {
		...
		if (env.getEffector() instanceof Player) {
			...
			if (((Player)env.getEffector()).isMagicalTypeClass()) {
				reduceTime = env.getEffector().getGameStats().getStat(StatEnum.BOOST_CASTING_TIME, 1000).getBonus();
			} else {
				reduceTime = env.getEffector().getGameStats().getAttackSpeed().getBase() - env.getEffector().getGameStats().getAttackSpeed().getCurrent();
			}
			for (ChargedSkill skill : env.getChargeSkillList()) {
				castTime += skill.getTime() - reduceTime;
			}
