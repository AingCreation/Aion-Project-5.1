...
public class ProvokerEffect extends ShieldEffect {
	...
	private void createProvokedEffect(final Creature effector, Creature target) {
		Skill skill = SkillEngine.getInstance().getSkill(effector, this.skillId, 1, target);
		if (skill != null) {
			if (skill.getSkillTemplate().getProperties().getTargetRelation() == TargetRelationAttribute.FRIEND && skill.getSkillTemplate().getSubType() == SkillSubType.ATTACK || skill.getSkillTemplate().getProperties().getTargetRelation() == TargetRelationAttribute.FRIEND && skill.getSkillTemplate().getTargetSlot() == SkillTargetSlot.DEBUFF || skill.getSkillTemplate().getProperties().getTargetMaxCount() <= 1) {
				SkillEngine.getInstance().applyEffectDirectly(this.skillId, effector, target, skill.getSkillTemplate().getEffectsDuration(skill.getSkillLevel()));
			} else {
