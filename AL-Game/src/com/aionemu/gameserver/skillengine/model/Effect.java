...
public class Effect implements StatOwner {
		...
		if (successEffects.isEmpty()) {
			...
			if (getSkillType() == SkillType.PHYSICAL) {
				if (getEffector() instanceof Player) {
					...
					if (pl.getPlayerClass() == PlayerClass.GUNSLINGER || pl.getPlayerClass() == PlayerClass.AETHERTECH) {
	...
	public void applyEffect() {
		...
		if (getEffected() instanceof Player) {
			if(((Player) getEffected()).getEffectController().hasAbnormalEffect(2918) && !isBuff()) {
				((Player) getEffected()).getEffectController().removeEffect(2918);
				effector.getEffectController().reflectEffect(effected, effector, getSkillId(), getSkillTemplate().getEffectsDuration(1));
				return;
			}
		} if (getEffected() instanceof Player) {
			if(((Player) getEffected()).getEffectController().hasAbnormalEffect(3272) && !isBuff()) {
				return;
			}
		}
	...
	private void schedulePeriodicActions() {
		...
		if (periodicActions.getChecktime() <= 0) {
			checktime = 1000;
		} else {
			checktime = periodicActions.getChecktime();
		}
	...
	public void setEffectStartTime() {
