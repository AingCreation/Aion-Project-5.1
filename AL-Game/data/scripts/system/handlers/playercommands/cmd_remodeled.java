 ...
 * @author Kashim
 */
public class cmd_remodeled extends PlayerCommand {

    public cmd_remodeled() {
        super("remo");
    }
    ...
    public void executeCommand(Player admin, String[] params) {
        ...
        if (params.length < 1) {
            PacketSendUtility.sendMessage(admin, "Syntax: .remo <itemid>\n");
            return;
        }

        if (params.length == 1) { // Use target
            int itemId = Integer.parseInt(params[0]);
            if (admin.getInventory().decreaseByItemId(186000178, 1)) {
                if (remodelItem(admin, itemId)) {
                    ...
                    ItemService.addItem(admin, 186000178, 1);
                }
            } else {
